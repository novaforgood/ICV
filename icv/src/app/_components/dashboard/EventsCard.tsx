...
const EventsCard: React.FC<EventCardProps> = ({
    ...
    const eventAssignee = String((event as { assigneeId?: string }).assigneeId ?? event.asigneeId ?? '') || '-'
    const eventCategory = String(event.contactCode) || ''
    const clientName = String(event.clientName ?? '') || '-'
    const clientId = String((event as { clientId?: string }).clientId ?? event.cliendId ?? '') || '-'
        ...
        >
            ...
            <div className="flex min-w-0 flex-1 items-center gap-6 overflow-hidden sm:min-w-[140px]">
                ...
                <div className="flex min-w-0 flex-1 flex-col gap-1 overflow-hidden text-start">
                    ...
                    <span className="truncate text-sm text-gray-600">{clientId}</span>
                </div>
            </div>

            {/* Group 2: Category + Assignee (fixed gap) */}
            <div className="flex min-w-0 shrink-0 flex-col items-start gap-2 overflow-hidden sm:flex-row sm:items-center sm:gap-6 sm:min-w-[140px]">
                <div
                    className={`${colorClass} flex h-[30px] min-w-0 max-w-full items-center overflow-hidden rounded-full px-3 py-1`}
                >
                    <span className="truncate text-sm font-medium">{eventCategory}</span>
                </div>
                <div className="flex min-w-0 items-center gap-2 overflow-hidden">
                    ...
                    />
                    <span className="min-w-0 truncate text-sm">{eventAssignee}</span>
                </div>
            </div>
        </Card>
    )
